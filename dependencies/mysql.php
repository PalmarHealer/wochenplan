...
function UpdateOrInsertLesson($type, $pdo, $id, $parent_lesson_id, $date_type, $new_date, $new_name, $new_description, $new_location, $new_time, $new_box_color, $new_notes, $new_assigned_user_id, $user_that_made_the_change, $disabled): bool {
...
function GetLessonInfo($day, $time, $room, $info, $pdo, $data = null) {
    if (!isset($data)) {
        $identifier = GetSetting("identifier", $pdo);
        if (str_contains($day, "-")) {
            $db_day = $day . " 00:00:00";
            $db_day2 = $day . " 23:59:59";
            $lessons = $pdo->prepare("SELECT * FROM angebot WHERE identifier = ? AND created_at <= ? AND (deleted_at >= ? OR deleted_at IS NULL) ORDER BY id DESC");
            $lessons->execute(array($identifier, $db_day, $db_day2));
        } else {
